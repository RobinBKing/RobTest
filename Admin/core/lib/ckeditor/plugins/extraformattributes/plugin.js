...
CKEDITOR.plugins.add('extraformattributes', {
    ...
    init: function (editor) {
        ...
        CKEDITOR.on('dialogDefinition', function (ev) {
            ...
            // 2016-06-20 / Brian B. / check to make sure we didn't already configure and add these "contents" Items
            var configComplete = false;
            for (var i = 0; i < dialogDefinition.contents.length; i++) {
                var loopContents = dialogDefinition.contents[i];
                if (loopContents != null && loopContents.id == 'formExtraOptions') {
                    configComplete = true;
                }
            }
            if (configComplete) {
